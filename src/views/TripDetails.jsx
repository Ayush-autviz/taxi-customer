...
const TripDetails = (props) => {
  ...
  useEffect(() => {
    ...
    const onDriverTracking = database()
      .ref(`/drivers/${data?.trip?.vehicle_type}/${data?.trip?.driver_id}`)
      .on("value", (snapshot) => {
        if (snapshot?.val()) {
          let marker = {
          ...
          if (data?.trip?.status <= 2) {
            get_direction(
              snapshot?.val()?.geo.lat + "," + snapshot.val().geo.lng,
    ...
    return onValueChange, onDriverTracking;
  }, []);

  const get_direction = async (startLoc, destinationLoc) => {
    try {
      let resp = await fetch(
        `https://maps.googleapis.com/maps/api/directions/json?origin=${startLoc}&destination=${destinationLoc}&key=${GOOGLE_KEY}`
      );
  ...
  const animate = (nextProps) => {
    ...
    if (driver_location !== nextProps) {
      if (Platform.OS === "android") {
        if (driver_track) {
          driver_track.animateMarkerToCoordinate(nextProps, duration);
        }
      } else {
        driver_location_ios
          .timing({
  ...
  const move_chat = () => {
    navigation.navigate("Chat", {
      trip_id: trip_id,
      first_name: data?.trip?.driver?.first_name,
    });
  };
        ...
        {data?.trip?.status <= 2 && (
          <Marker coordinate={home_marker}>
            <Image
              ...
              source={require(".././assets/img/tracking/home.png")}
            />
          </Marker>
        )}
        {data?.trip?.status >= 2 && (
          <Marker coordinate={destination_marker}>
            <Image
              style={{ height: 30, width: 25 }}
              source={require(".././assets/img/tracking/destination.png")}
            />
          </Marker>
        )}
        <MarkerAnimated
          ...
          rotation={bearing}
          coordinate={
            Platform.OS === "ios" ? driver_location_ios : driver_location
          }
          identifier={"mk1"}
        >
            ...
            {on_load == 1 ? (
              <View style={{ padding: 10 }}>
                ...
                >
                  ...
                  >
                    Your OTP code is : #{data?.trip?.otp}
                  </Text>
                </View>
        ...
        <DialogContent>
          <FlatList
            ...
            renderItem={({ item, index }) => (
              <TouchableOpacity
                onPress={call_trip_cancel.bind(this, item.id, item.type)}
                activeOpacity={1}
              >
                <View style={{ padding: 10 }}>
                  <Text
